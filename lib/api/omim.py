...
class Omim:
    def __init__(self, api_key='', mimdir='data', use_cached=True, config = None):
        '''
        ...
        if config:
            ...
            self._mimdir = mimdir
        mim2gene = self._retrieve_file(os.path.join(mimdir,'mim2gene.txt')
                ,'https://omim.org/static/omim/data/mim2gene.txt'
                ,['mim_number','mim_entry_type','entrez_id', 'gene_symbol','ensembl'])
        self.mim2gene = mim2gene.dropna(subset=['mim_number', 'entrez_id']).set_index('mim_number')
        self.entrez2gene = mim2gene.dropna(subset=['mim_number', 'entrez_id']).set_index('entrez_id')

        morbidmap_url = 'https://data.omim.org/downloads/{}/morbidmap.txt'.format(api_key)
        morbidmap = self._retrieve_file(os.path.join(mimdir,'morbidmap.txt'), morbidmap_url
                ,['phenotype', 'gene_symbol', 'mim_number', 'cyto_location'])
        morbidmap['phen_mim_number'] = morbidmap['phenotype'].apply(self._extract_omim)
        self.morbidmap = morbidmap

        self.phen_to_mim = self._create_phen_to_mim()

    def _create_phen_to_mim(self):
        # phen_rows = self.morbidmap.loc[self.morbidmap['phen_mim_number'].notna()]
    ...
    def _search_single(self, dataframe, column, needle, target):
        ...
        '''
        # pandas saves values as object type per default, coerce both to str to make
        # comparisons easier to debug correctly
    ...
    def mim_gene_to_entrez_id(self, mim_gene):
        # r = self._search_single(self.mim2gene, 'mim_number', mim_gene, 'entrez_id')
        try:
    ...
    def mim_gene_to_symbol(self, mim_gene):
        # r =  self._search_single(self.mim2gene, 'mim_number', mim_gene, 'gene_symbol')
        r = self.mim2gene.at[str(mim_gene), 'gene_symbol']
        return r

    def entrez_id_to_mim_gene(self, entrez_id):
        # r = self._search_single(self.mim2gene, 'entrez_id', entrez_id, 'mim_number')
        try:
