...
def checkjsons(step):
    ...
    with open(location+'Results/result_'+step+date+'.json', 'w') as dicttojson:
...
wantto=raw_input('Make a result file? (y OR n) ')
...
if wantto!='n':
    ...
    # Liste mit korrekten JSONs ohne VCF
    with open(location+'Results/result_'+step+date+'.json') as json_data:
    ...
    for file in os.listdir(location+'current_serverstatus/'):
        if 'json' in file and file in novcf_list:
            with open(location+'current_serverstatus/'+file) as json_data:
                d=json.load(json_data)
                if d['vcf']!='noVCF':
                    vcfs=vcfs+1
    ...
    with open(location+'Results/JsonsVCF_'+date+'.vcf', 'w') as outfile:
